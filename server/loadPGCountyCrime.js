...
export async function loadPGCountyCrimeData(req, res, next) {
    try {
      ...
      console.log('Results in loadPGCountyCrimeData middleware', json.length); // let's check that something's there before we return it
      req.PGCountyCrimeData = reply; // and let's _attach_ the data to our request object here
      next();
    } catch (err) {
